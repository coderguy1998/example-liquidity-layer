...
pub struct RedeemCctpFill<'info> {
    ...
    to_router_endpoint: Box<Account<'info, matching_engine::state::RouterEndpoint>>,
...
fn handle_redeem_fill_cctp(ctx: Context<RedeemCctpFill>, args: CctpMessageArgs) -> Result<()> {
    let fill_vaa = wormhole_cctp_solana::cpi::verify_vaa_and_mint(
    ...
    let endpoint = &ctx.accounts.to_router_endpoint;
    let emitter = fill_vaa.emitter_info();
    require_eq!(
        emitter.chain,
        endpoint.chain,
    ...
    let deposit = WormholeCctpPayload::try_from(fill_vaa.payload())
        .unwrap()
        .message()
